...
namespace collections {
    ...
    template<typename T>
    void LinkedList<T>::cloneFrom(const LinkedList<T>& other) {
    ...
    template<typename T>
    LinkedList<T>::LinkedList(const std::initializer_list<T> init)
    ...
    template<typename T>
    void LinkedList<T>::insert(const size_t index, const T& val) {
        if (index > count) {
    ...
    template<typename T>
    int LinkedList<T>::indexOf(const T& val) const {
        ListNode<T>* current = first;
        size_t idx = 0;
        while (current != nullptr) {
    ...
    template<typename T>
    void LinkedList<T>::setAt(const size_t index, const T& val) {
        if (index >= count) {
            throw std::out_of_range("Индекс выходит за пределы списка");
        }
        ListNode<T>* current = first;
    ...
    template<typename T>
    std::string LinkedList<T>::asString() const {
        std::stringstream ss;
        ss << "[";
        ListNode<T>* current = first;
        while (current != nullptr) {
